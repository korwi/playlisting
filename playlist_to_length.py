...
def url_to_id(x):
    ...
    if re.match(r"^playlist", str(x)[24:]) and re.match(r"^list=", str(x)[33:]) and re.match(r"^https://www.youtube.com", str(x)) and (len(x)==72): 
        pass
    else: 
        return "invalid url, not a youtube url or not a youtube playlist"
    # finally return id if thats so the case
    return str(x)[-34:]
